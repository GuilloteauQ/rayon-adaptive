...
impl<'a, T: 'a + Ord + Sync + Copy + Send> SortingSlices<'a, T> {
    ...
    fn fuse(self, mid: Self, right: Self, block_size_fuse: usize) -> Self {
        ...
        let destination_index = if left.i == mid.i
            && mid.i == right.i
            && left.s[left.i].last() <= mid.s[mid.i].first()
            && mid.s[mid.i].last() <= right.s[right.i].first()
        {
            left.i
        } else {
...

            {
                ...
                if left_input.last() < mid_input.first() && mid_input.last() < right_input.first() {
                    // A B C
                    output_slice[..left_input.len()].copy_from_slice(left_input);
                    output_slice[left_input.len()..left_input.len() + mid_input.len()]
                        .copy_from_slice(mid_input);
                    output_slice[left_input.len() + mid_input.len()..].copy_from_slice(right_input);
                } else if right_input.last() < mid_input.first()
                    && mid_input.last() < left_input.first()
                {
                    // C B A
                    output_slice[..right_input.len()].copy_from_slice(right_input);
                    output_slice[right_input.len()..right_input.len() + mid_input.len()]
                        .copy_from_slice(mid_input);
                    output_slice[right_input.len() + mid_input.len()..].copy_from_slice(left_input);
                } else if left_input.last() < right_input.first()
                    && right_input.last() < mid_input.first()
                {
                    // A C B
                    output_slice[..left_input.len()].copy_from_slice(left_input);
                    output_slice[left_input.len()..left_input.len() + right_input.len()]
                        .copy_from_slice(right_input);
                    output_slice[right_input.len() + left_input.len()..].copy_from_slice(mid_input);
                } else if mid_input.last() < right_input.first()
                    && right_input.last() < left_input.first()
                {
                    // B C A
                    output_slice[..mid_input.len()].copy_from_slice(mid_input);
                    output_slice[mid_input.len()..mid_input.len() + right_input.len()]
                        .copy_from_slice(right_input);
                    output_slice[right_input.len() + mid_input.len()..].copy_from_slice(left_input);
                } else if mid_input.last() < left_input.first()
                    && left_input.last() < right_input.first()
                {
                    // B A C
                    output_slice[..mid_input.len()].copy_from_slice(mid_input);
                    output_slice[mid_input.len()..mid_input.len() + left_input.len()]
                        .copy_from_slice(left_input);
                    output_slice[left_input.len() + mid_input.len()..].copy_from_slice(right_input);
                } else if right_input.last() < left_input.first()
                    && left_input.last() < mid_input.first()
                {
                    // C A B
                    output_slice[..right_input.len()].copy_from_slice(right_input);
                    output_slice[right_input.len()..right_input.len() + left_input.len()]
                        .copy_from_slice(left_input);
                    output_slice[left_input.len() + right_input.len()..].copy_from_slice(mid_input);
                } else {
...
pub fn adaptive_sort_join3_swap<T: Ord + Copy + Send + Sync>(
    slice: &mut [T],
    block_size: usize,
