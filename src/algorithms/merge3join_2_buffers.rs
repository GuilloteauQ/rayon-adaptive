...
pub fn adaptive_sort_join3_2_buffers<T: Ord + Copy + Send + Sync>(
    slice: &mut [T],
    block_size: usize,
    ...
    let new_block_size = {
        let recursions =
            (((len as f64) / (block_size as f64)).log(3.0).ceil() / 2.0 - 0.5).ceil() as usize * 2;
        ((len as f64) / (recursions as f64).powf(3.0)).ceil() as usize + 2
    };
