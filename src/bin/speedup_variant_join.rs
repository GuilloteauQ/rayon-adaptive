...
fn main() {
    ...
    .chain(once((
        Box::new(move |mut v: Vec<u32>| adaptive_sort_join3(&mut v, block_size, block_size_fuse))
            as Box<Fn(Vec<u32>) + Sync + Send>,
        format!("3/3-Join"),
    )))
    .chain(once((
        Box::new(move |mut v: Vec<u32>| adaptive_sort_join2(&mut v, block_size, block_size_fuse))
            as Box<Fn(Vec<u32>) + Sync + Send>,
        format!("2/2-Join"),
    )))
    .chain(once((
        Box::new(move |mut v: Vec<u32>| {
            adaptive_sort_join3_by_2(&mut v, block_size, block_size_fuse)
        }) as Box<Fn(Vec<u32>) + Sync + Send>,
        format!("3/2-Join"),
    )))
    .chain(once((
        Box::new(|mut v: Vec<u32>| v.par_sort()) as Box<Fn(Vec<u32>) + Sync + Send>,
        "Rayon".to_string(),
    )))
    .collect();
